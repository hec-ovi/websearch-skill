...
Every inter-layer message and CLI `--json` output is a JSON Schema 2020-12 document. The schema is the load-bearing isolation: a layer
...
| `search.schema.json` | Layer 1 search (`SearchRequest`, `SearchPayload`, `ResultItem`, `SourceProvenance`) | 1.2.0 | frozen |
| `fetch.schema.json` | Layer 2A fetch sub-port (`FetchRequest`, `FetchResult`) | 1.3.0 | frozen |
...
| `agent-io.schema.json` | Layer 3 agent I/O (`AgentSearchRequest`/`Payload`, `AgentFetchRequest`/`AgentOpenRequest`/`AgentPage`/`AgentFetchPayload`, `FenceInfo`) | 1.2.0 | frozen |
...
| `doctor.schema.json` | Diagnostics: per-capability self-test (`DoctorRequest`, `OptionalLayer`, `CheckResult`, `DoctorSummary`, `DoctorPayload`) | 2.1.0 | frozen |
...
`arxiv.schema.json` and `github.schema.json` are standalone extra tools, not part of
the search-fetch-format pipeline, but they emit the same `Envelope` (meta.layer
`arxiv` / `github`) so the CLI and MCP faces handle them uniformly. Both are keyless:
arXiv over the official Atom API, GitHub over the unauthenticated REST search API.

`doctor.schema.json` is diagnostics over the same Envelope (meta.layer `doctor`). One
rule to know when consuming it: `Envelope.ok` says the diagnostic RAN, not that the
installation is healthy. A run that finds ten failures is still `ok: true` with a full
payload; health is `data.summary.fail == 0`, mirrored on `meta.healthy`.
